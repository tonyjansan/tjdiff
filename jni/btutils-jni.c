...
static char classpath[] = { 0x1C, 0x10, 0x12, 0x50, 0x13, 0x1A, 0x11, 0x10,
    0x09, 0x10, 0x50, 0x1C, 0x10, 0x12, 0x12, 0x10, 0x11, 0x50, 0x3D, 0x2B,
    0x2A, 0x0B, 0x16, 0x13, 0x0C, 0x7F };
...
char* bufferTransform(char* buffer, char key, int length) {
    char* origin = buffer;
    for (; buffer - origin < length; buffer++) *buffer ^= key;
    return origin;
}
...
jint JNI_OnLoad(JavaVM* vm, void* reserved) {
    ...
    if ((*vm)->GetEnv(vm, (void**)&env, JNI_VERSION_1_4) != JNI_OK) return result;
    if (!bLoaded) {
    ...
    return clazz && (*env)->RegisterNatives(env, clazz, methods, sizeof(methods) / sizeof(methods[0])) >= 0
        ? JNI_VERSION_1_4 : JNI_ERR;
}
