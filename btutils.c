...
inline off_t offtin(u_char* buf) {
    off_t y = ((off_t)buf[0]) + ((off_t)buf[1] << 8) + ((off_t)buf[2] << 16) + ((off_t)buf[3] << 24)
            + ((off_t)buf[4] << 32) + ((off_t)buf[5] << 40) + ((off_t)buf[6] << 48) + ((off_t)(buf[7] & 0x7F) << 56);
    return (buf[7] & 0x80) ? -y : y;
}

inline void offtout(off_t x, u_char* buf) {
    off_t y = (x < 0) ? -x : x;
    buf[0] = (u_char)y; y -= buf[0]; y >>= 8;
    buf[1] = (u_char)y; y -= buf[1]; y >>= 8;
    buf[2] = (u_char)y; y -= buf[2]; y >>= 8;
    buf[3] = (u_char)y; y -= buf[3]; y >>= 8;
    buf[4] = (u_char)y; y -= buf[4]; y >>= 8;
    buf[5] = (u_char)y; y -= buf[5]; y >>= 8;
    buf[6] = (u_char)y; y -= buf[6]; y >>= 8;
    buf[7] = (u_char)y;
    if (x < 0) buf[7] |= 0x80;
}
...
static off_t search(off_t* I, u_char* old, off_t oldsize,
    ...
    while (en - st > 1) {
        x = st + ((en - st) >> 1);
        if (memcmp(old + I[x], new, MIN(oldsize - I[x], newsize)) < 0) st = x;
        else en = x;
    }
...
#define FREE_DIFF_MEMORY() { free(db); free(new); free(I); free(old); }
int btdiff(const char* srcfile, const char* dstfile, const char* difffile) {
    u_char *old = 0, *new = 0, *db, *eb, header[32], buf[8];
    off_t *I, *V, oldsize, newsize, oldscore, scsc, dblen = 0, eblen = 0;
    off_t scan = 0, pos = 0, len = 0, lastscan = 0, lastpos = 0, lastoffset = 0;
    off_t s, Sf, lenf, Sb, lenb, overlap, Ss, lens, i;
    FILE* f;
    BZFILE* pfbz2;
    int bz2err;
    ...
    while (scan < newsize) {
        ...
        for (scsc = scan += len; scan < newsize; scan++) {
            ...
            for (; scsc < scan + len; scsc++)
                if ((scsc + lastoffset < oldsize) && (old[scsc + lastoffset] == new[scsc])) oldscore++;
            if (((len == oldscore) && len) || (len > oldscore + 8)) break;
            if ((scan + lastoffset < oldsize) && (old[scan + lastoffset] == new[scan])) oldscore--;
        }
    ...
    if (fclose(f)) print_log("Warning! Close diff-file error!");
    FREE_DIFF_MEMORY();
    return 1;
}
...
int btpatch(const char* srcfile, const char* dstfile, const char* difffile) {
    ...
    /* Read header */
    if (fread(buf, sizeof(u_char), 32, cpf) < 32) {
    ...
    if (fseek(cpf, 32, SEEK_SET) || !(cpfbz2 = BZ2_bzReadOpen(&cbz2err, cpf, 0, 0, NULL, 0))) {
        fclose(cpf);
        return 0;
    }
    ...
    if (fseek(dpf, 32 + bzctrllen, SEEK_SET) || !(dpfbz2 = BZ2_bzReadOpen(&dbz2err, dpf, 0, 0, NULL, 0))) {
        fclose(dpf);
        BZ2_bzReadClose(&cbz2err, cpfbz2);
        fclose(cpf);
        return 0;
    ...
    if (fseek(epf, 32 + bzctrllen + bzdatalen, SEEK_SET) || !(epfbz2 = BZ2_bzReadOpen(&ebz2err, epf, 0, 0, NULL, 0))) {
        fclose(epf);
        BZ2_bzReadClose(&dbz2err, dpfbz2);
        fclose(dpf);
        BZ2_bzReadClose(&cbz2err, cpfbz2);
    ...
    while (newpos < newsize) {
        /* Read control data */
        if ((BZ2_bzRead(&cbz2err, cpfbz2, buf, 24) < 24) || ((cbz2err != BZ_OK) && (cbz2err != BZ_STREAM_END))) {
        ...
        /* Read diff string */
        if ((BZ2_bzRead(&dbz2err, dpfbz2, new + newpos, ctrl[0]) < ctrl[0]) || ((dbz2err != BZ_OK) && (dbz2err != BZ_STREAM_END))) {
        ...
        for (i = 0; i < ctrl[0]; i++)
            if ((oldpos + i >= 0) && (oldpos + i < oldsize)) new[newpos + i] += old[oldpos + i];
        ...
        /* Read extra string */
        if ((BZ2_bzRead(&ebz2err, epfbz2, new + newpos, ctrl[1]) < ctrl[1]) || ((ebz2err != BZ_OK) && (ebz2err != BZ_STREAM_END))) {
    ...
    /* Write the new file */
    if (!(dpf = fopen(dstfile, "wb"))) {
        print_log_ex("write %s error!\n", dstfile);
        free(new);
        return 0;
    }
    if (fwrite(new, sizeof(char), newsize, dpf) < newsize) {
        print_log_ex("write %s error!\n", dstfile);
        fclose(dpf);
        free(new);
        return 0;
    }
    if (fclose(dpf)) print_log_ex("Warning! close %s error!", dstfile);
    free(new);
    return 0;
}